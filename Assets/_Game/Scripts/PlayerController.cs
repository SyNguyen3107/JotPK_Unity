...
public class PlayerController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Lưu ý: Việc load item từ save game sẽ do GameManager chủ động gọi SetStoredItem
    }
    ...
    #region --- SAVE / LOAD SUPPORT (MỚI) ---
    // GameManager gọi hàm này khi load game để khôi phục item
    public void SetStoredItem(PowerUpType itemType)
    {
        if (GameManager.Instance == null) return;

        // Tìm Data object dựa trên Enum Type (từ danh sách drop của GM)
    ...
    public void PickUpItem(PowerUpData newItem)
    {
        ...
        if (heldItem != null)
        {
            ActivateItem(newItem); // Dùng luôn item mới nếu đang cầm 1 cái rồi
    ...
    void ApplyBuffDuration(PowerUpData item)
    {
        ...
        Debug.Log($"Buff Activated: {item.itemName}");
    }
    #endregion
    ...
    public void TriggerDeathAnimation()
    {
        ...
        SetVisualState(isIdle: false); // Show active object to play death anim
